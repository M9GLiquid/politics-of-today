...
import { buildSessionUserForUserId } from "@/lib/session-user";
...
export function applySessionCookieToResponse(
  ...
  response.cookies.set(SESSION_COOKIE_NAME, token, {
    ...sessionCookieBase(),
    maxAge: AUTH_CONFIG.sessionMaxAgeSec,
  });
}
...
export async function signSession(user: SessionUser): Promise<string> {
  ...
  return new jose.SignJWT(body)
    ...
    .setIssuedAt()
    .setExpirationTime(
      Math.floor(Date.now() / 1000) + AUTH_CONFIG.sessionMaxAgeSec,
    )
    .sign(secret);
}
...
export { buildSessionUserForUserId } from "@/lib/session-user";
